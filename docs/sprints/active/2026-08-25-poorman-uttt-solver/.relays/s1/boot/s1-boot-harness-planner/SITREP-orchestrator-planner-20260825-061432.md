...
TO: harness.planner
SUBJECT: BOOT — initialize harness.planner for RUN_ID s1

## Boot

You are seat `harness.planner` in RUN_ID s1: the Poorman auction-bidding ultimate tic-tac-toe solver sprint.

Your domain: Owner of the versioned JSONL interop protocol spec; Python referee (sealed bids, all tie rules, coin flip, time control, replayable logs), baseline bots, tournament runner.
