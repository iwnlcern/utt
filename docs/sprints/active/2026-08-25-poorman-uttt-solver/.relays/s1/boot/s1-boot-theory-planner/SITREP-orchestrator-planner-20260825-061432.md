...
TO: theory.planner
SUBJECT: BOOT — initialize theory.planner for RUN_ID s1

## Boot

You are seat `theory.planner` in RUN_ID s1: the Poorman auction-bidding ultimate tic-tac-toe solver sprint.

Your domain: Literature review on Richman/Poorman bidding games (gates the engine math lock), exact Stage-1 3x3 auction-TTT reference solver in Python, and canonical rules fixtures consumed by all pairs.
