...
DISPATCH_ID: s1-boot-engine-implementer
RUN_ID: s1
...
SUBJECT: BOOT — initialize engine.implementer for RUN_ID s1

## Boot

You are seat `engine.implementer` in RUN_ID s1: the Poorman auction-bidding ultimate tic-tac-toe solver sprint.

Your domain: C++26 CPU engine: bitboard rules core, Poorman threshold-backup search, transposition table, move ordering, handcrafted threshold evaluator, root bid matrix, JSONL protocol I/O. Search-math design locks only after theory's review.
