...
Revision: 13 (2026-08-27; folds engine-c4-design-review-12 M1–M2 — the certified branch gains its mechanically checkable predicate (exact ceiling invariance across the sound r-enclosure plus interval extremizer dominance) and the analyze rejection object gains its wire schema; rev 12 folded review-11 M1–M3 — the forced-branch bid becomes a CERTIFIED action carrier gated on exact-quality aggregates (the midpoint-anchor bid was unsound, witness recorded), the play-mode window gets its literal outward-rounded construction with the exact gate retained as sole classification authority, and the analyze mode gets a locked resource contract; rev 11 was amendment 2 folding the four-lens panel on PR #10 — ply-0 fair-coin tie semantics per R1, staged-best-move publication, and the acceptance-hardening block; G1–G6 and all solved-scale exactness are unchanged)
...
- Payoff entries (theory-parity semantics): the higher bid wins (tie → `h` moves); the child is the winner's move applied; the entry is the child's outcome at the POST-PAYMENT budgets.
  ...
  - CERTIFIED branch (mechanically checkable predicate — review-12 M1 fold; ulp proximity NEVER certifies, because `ceil` is discontinuous: a one-ulp enclosure straddling `k/M` has endpoint ceilings 1 and 2 at `r = 1/4, M = 4`): the C8a action (bid `k` plus the extremizing conditional move) is published ONLY when ALL of the following hold, each decided by executable checks with no runtime rationals:
    (i) both root aggregates are COMPLETE and SOUND (full-width, no widening, no evaluator taint — quality `exact` or `bound` with both one-sided contributions sound);
    (ii) CEILING INVARIANCE: the sound critical-fraction enclosure `[r_down(a.hi, b.lo), r_up(a.lo, b.hi)]` (crossed endpoints per `r`'s monotonicity — increasing in `b`, decreasing in `a` — via the section-2 directed primitives) yields ONE integer under the EXACT bit-decomposition ceiling (the P2 gate's integer machinery with a ceiling variant: decompose each endpoint as `m·2^e`, compute `ceil(m·2^e·M)` exactly in `__int128`): `k = ceil_exact(r_enc.lo · M) == ceil_exact(r_enc.hi · M)` — since `ceil` is monotone, equality pins the theorem's true `ceil(r · M)` exactly;
    (iii) EXTREMIZER DOMINANCE: the selected X-child satisfies `child.hi <= other.lo` for EVERY other X-child (then `true_selected <= child.hi <= other.lo <= true_other`, so the selected child attains the true minimum — sound including ties), symmetrically `child.lo >= other.hi` for O;
    (iv) ordered branch proven by intervals (`a.hi <= b.lo`); the certified ZUGZWANG branch instead requires `a.lo > b.hi` and publishes bid 0 with the tie-owner-side dominant child per F-C3 (dominance check on that side).
    `k` is clamped to `[0, own stack]` with C8a's affordability; X and O sides symmetric on their respective certificates.
  - HEURISTIC branch: when the gate fires but ANY predicate above fails (including ceiling straddle, overlapping child intervals, widening, or cancellation mid-aggregation), the forced classification is still REPORTED (the gate's value claim is sound on the adverse endpoint regardless of action), but the published ACTION comes from the ordinary matrix path within the reserve; matrix incompleteness falls back to staged publication as everywhere else.
  - Acceptance: a mutation-sensitive battery pins BOTH branches — the review-11 inexact-aggregate witness (must route to the matrix, must NOT publish 14), an ulp-enclosure STRADDLING `k/M` (must route to the matrix), OVERLAPPING exact-quality child intervals (must route to the matrix), and a dominant-child stable-ceiling case publishing the pinned `k` with the dominant move; an injected-point `r` check alone is insufficient.
  ...
  Analyze-mode resource contract (amendment 2, per review-11 M3; wire schema per review-12 M2): `uttt_engine analyze` emits EXACTLY ONE stdout JSON object per input line, preserving request↔result cardinality and order.
  A rejected line (`depth > 12`, or any malformed request) emits the rejection object `{"error": "<reason string>", "line": <1-based input line number>}` on STDOUT — the `error` key is the discriminator and never appears in a success object — and processing CONTINUES with the next line; stderr carries nothing for per-line rejections; the process exit code stays 0 when at least the stream was readable.
  Accepted requests run with a fixed node cap of `5,000,000` and no other stop source; an over-budget search terminates at the cap and returns the SCHEMA-VALID result of the last completed iteration with `complete: false`; the success schema is byte-compatible with the existing fitter/P1b consumers (no field change).
  `fit_eval.py` treats any `error`-discriminated line in its corpus run as a HARD failure of the fit (its committed corpus uses depth 6 and must produce zero rejections).
  Acceptance: the named E2 case runs the ordered stream `valid, depth-13, valid` and asserts three stdout objects in order — success, rejection object with the correct `line`, success — plus the adversarial-depth bounded-termination case.
