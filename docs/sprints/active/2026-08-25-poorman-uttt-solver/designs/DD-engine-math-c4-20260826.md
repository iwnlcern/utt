...
- Payoff entries (theory-parity semantics): the higher bid wins (tie → `h` moves); the child is the winner's move applied; the entry is the child's outcome at the POST-PAYMENT budgets.
  ...
  Zero-total totality (M1 fold): terminal authority applies FIRST (a terminal child classifies exactly regardless of budgets); for a NONTERMINAL child at the reachable post-payment coordinate `M2 = 0` (an all-in winning bid pays the entire remaining stack), define `p := 1/2` — derived from F-C5's chip-comparison semantics, where exact chip equality is the half-half draw point and zero-zero is exact equality — so the payoff is the finite `clamp(κ · (1/2 − t_est_child), −1, +1)`; the payoff function is therefore TOTAL on every legal coordinate, every stored value is validated finite in `[-1, +1]`, and no division by zero exists on any path.
