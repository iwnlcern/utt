...
Revision: 8 (2026-08-27; resolves engine-c4-design-review-7's human-decision-required — the operator overturned the interim `p := 1/2` convention after identifying the true reduction and ratified the alternation-game estimate as G6; rev 7 had folded review-6 M1–M4 into amendment 1 — graded estimate payoff, operator-ratified G5, resolving engine-c4-impl-blocker-1; G1–G5 and all solved-scale exactness are unchanged)
...
- Payoff entries (theory-parity semantics): the higher bid wins (tie → `h` moves); the child is the winner's move applied; the entry is the child's outcome at the POST-PAYMENT budgets.
  ...
  The payoff entry is therefore the ALTERNATION-GAME EVALUATION: the existing search runs on the degenerate zero-budget model — one mover per ply (the tie owner), `h` flipping per F-C4, the same depth/clock limits, the evaluator at horizon — and the entry is that game's outcome value in `[-1, +1]` directly, with NO `p` and no `κ` scaling on this branch; it is EXACT (ordinal `-1/0/+1`) when the sub-search resolves to terminals full-width, else estimate-tainted like any other in-band entry.
  The payoff function is thereby TOTAL on every legal coordinate, every stored value is validated finite in `[-1, +1]`, and no division by zero exists on any path; the sub-search cost is bounded by the same per-child depth/clock budget as every other matrix child.
