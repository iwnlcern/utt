...
Revision: 4 (2026-08-26; folds engine-c4-design-review-3 residuals M1–M3 atop the rev-2/rev-3 folds, all verified against sources, none contested)
...
- Payoff entries (theory-parity semantics): the higher bid wins (tie → `h` moves); the child is the winner's move applied; the entry is the child's outcome at the POST-PAYMENT budgets, mapped to `{-1, 0, +1}` exactly as the reference maps `{0,1,2} - 1`.
  Child-value backend seam (M3 fold — outward-rounded intervals are NEVER degenerate, even full-width, so they cannot source exact payoffs): the payoff computation consumes child values through a value-provider seam on the GameModel.
  Production binds the TInterval backend: a child forced by this section's exact gate at the post-payment budgets (or a terminal) yields an exact ordinal entry; an in-band child entry uses the interval midpoint's side and taints the root report to `quality = estimate` — this fallback is never relabeled exact.
  The ttt3 acceptance harness binds a TEST-SIDE EXACT-RATIONAL backend (the same exact evaluator A3 already requires) implementing identical F/zugzwang/terminal semantics in rational arithmetic — feasible at solved ttt3 scales — so EVERY solved-scale matrix entry is an exact discrete outcome with no ambiguous case, while the identical action-construction, winner-resolution, and post-payment transition code runs under both bindings.
  This exact backend is also the P1a authority bridge: solved-scale classifications, including equality/knife-edge entries, come from it, never from float intervals.
...
- A8 root matrix parity (M3 fold): the engine's candidate action set must equal the theory reference's action labels EXACTLY on the fixture states, and the payoff matrices must agree entry-wise (exact rational comparison at fixture scales) BEFORE solver outputs are compared; every solved-scale payoff entry — including equality/knife-edge entries — is sourced from the exact-rational test backend (section 7 seam), never from float intervals, so the entry-wise comparison is exact with no ambiguous case; then averaged-profile exploitability ≤ a PLAN-committed epsilon and agreement with theory's exact LP values within the locked tolerance — E2.
