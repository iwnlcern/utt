...
Revision: 3 (2026-08-26; folds engine-c4-design-review-2 residuals M1–M5 atop the rev-2 fold, all verified against sources, none contested)
...
Byte contract (every byte an explicit field or named padding; `static_assert(sizeof == 32 && alignof >= 8)` plus per-field `offsetof` asserts):

- `tag` @0 (u32) — the independent 32-bit Zobrist fold of rules-DD §5 (separate random table, not a truncation of the 64-bit key).
- `smp_word` @4 (u32) — the future SMP publication word, defined ONCE here as the XOR of `tag` with the low 32 bits of `lo`'s bit pattern when the publication discipline is enabled; in s1 it is written as literal zero and ignored on probe.
- `lo` @8, `hi` @16 (f64 each).
...
- Payoff entries (theory-parity semantics): the higher bid wins (tie → `h` moves); the child is the winner's move applied; the entry is the child's outcome at the POST-PAYMENT budgets, mapped to `{-1, 0, +1}` exactly as the reference maps `{0,1,2} - 1`.
  The engine computes that outcome from the child's threshold interval via this section's exact gate at the post-payment budgets (terminal children classify directly); at fixture scales, full-width search yields degenerate intervals, so every entry is exact and A8's byte-wise matrix comparison is executable; at production scales an in-band child entry uses the interval midpoint's side and taints the root report to `quality = estimate`.
