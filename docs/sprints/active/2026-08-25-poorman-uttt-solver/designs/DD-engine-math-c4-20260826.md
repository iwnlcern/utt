...
Revision: 11 (2026-08-27; amendment 2 — folds the four-lens adversarial panel on PR #10 @ 5907f93: ply-0 coin-tie matrix semantics (fair-coin expectation per R1, replacing unratified own-seat optimism), the P2 forced-branch bid locked to F-C8a's certificate strategy, staged publication clarified to include the staged best move, and the panel's acceptance-hardening additions; rev 10 closed the collision-safe alternation memo; G1–G6 and all solved-scale exactness are unchanged)
...
- Payoff entries (theory-parity semantics): the higher bid wins (tie → `h` moves); the child is the winner's move applied; the entry is the child's outcome at the POST-PAYMENT budgets.
  ...
  P2 forced-branch bid (amendment 2 — panel finding; replaces the ad-hoc `min(stack, E+1)` flat bid, which can squander a certificate: witness `E = 4, T = 0.2, bx = 25, M = 100` — the gate fires at margin 21 > 20, yet four flat bids of 5 can end at `bx = 5 < bo = 75` and lose the F-C5 chip comparison): when the gate fires, the published bid is F-C8a's certificate strategy `k = ceil(r · M)` from the root anchors' critical fraction, computed by the exact rational reading (ceiling, distinct from the matrix `k*`'s half-up) and clamped to `[0, own stack]`; C8a's induction is precisely the proof that this bid preserves the winning margin ply over ply.
  ...
  Play-mode cutoffs (amendment 2 — conformance restated, not new semantics): §4's sentence "in play mode the root layer narrows it to the P2 decision band" is NOT optional — production play passes that window so the cut machinery is active; an acceptance row asserts cuts are enabled in a production-shaped run.
