...
Revision: 9 (2026-08-27; folds engine-c4-design-review-8 findings M1–M3 — the G6 alternation solver is now locked as a dedicated max/min recurrence with a defined horizon score, semantic isolation from the threshold TT, per-unique-child cost bounds, and exact scale-zero oracle acceptance; rev 8 had recorded operator decision G6 overturning the interim `p := 1/2` convention; G1–G6 and all solved-scale exactness are unchanged)
...
- Payoff entries (theory-parity semantics): the higher bid wins (tie → `h` moves); the child is the winner's move applied; the entry is the child's outcome at the POST-PAYMENT budgets.
  ...
  - Semantic isolation (review-8 M2 fold): the alternation solver NEVER reads or writes the persistent threshold TT — its memo is a PRIVATE per-matrix-construction table keyed by `(state key, h)` carrying `{double score; bool exact; uint8 depth}`, discarded with the matrix; the frozen 32-byte threshold-TT layout and its value domain remain untouched, and A9 proves the threshold table's contents are bit-identical before/after a zero-total sub-search.
