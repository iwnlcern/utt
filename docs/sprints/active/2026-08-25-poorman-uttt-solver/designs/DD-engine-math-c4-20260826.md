...
Revision: 6 (2026-08-27; amendment 1 — graded estimate payoff, operator-ratified, resolving engine-c4-impl-blocker-1: the rev-5 sound-midpoint in-band ordinal is information-free at real horizons because unresolved children carry sound `[0,1]` intervals, collapsing the production matrix to coarse ternary rows and defeating A10; G1–G4 and all solved-scale exactness are unchanged)
...
- Payoff entries (theory-parity semantics): the higher bid wins (tie → `h` moves); the child is the winner's move applied; the entry is the child's outcome at the POST-PAYMENT budgets, mapped to `{-1, 0, +1}` exactly as the reference maps `{0,1,2} - 1`.
  ...
  In-band production payoff (amendment 1, operator-ratified — replaces the rev-5 sound-midpoint sign, which is information-free when the sound interval is `[0,1]`): an in-band child yields the CONTINUOUS payoff `clamp(κ · (p − t_est_child), −1, +1)` with `p = bx2 / M2` and `κ` a PLAN-committed scaling constant (default `κ = 8`, saturating at `|p − t_est| ≥ 0.125`), computed in plain double arithmetic — permissible here because the entry is declared `exact = false` and estimate-quality guidance, never a classification (the exact-comparison machinery remains mandatory at the P2 gate and for exact ordinals); any such entry taints the root report to `quality = estimate`, never relabeled exact.
  `t_est` (estimate center): `SearchResult` additionally carries `double t_est` — for exact/bound-quality nodes it is the sound-interval midpoint; for estimate-quality nodes it is the evaluator-informed center propagated through the live search by scalar F/zugzwang point arithmetic alongside the interval backup; a pure TT hit, whose 32-byte entry stores no estimate center (frozen layout), uses its stored interval midpoint as `t_est` — conservative and disclosed; the root matrix children are searched live, so their `t_est` values carry real evaluator information.
  Averaged-profile action extraction tie-break (amendment 1): among maximum-probability actions, prefer the higher expected payoff against the opponent's averaged profile, then the lower bid, then the lower move index — deterministic, replacing first-index selection.
...
- A9 metadata honesty: a widened node never reports `exact` (mutation-guarded); estimate taint propagates; P1b outputs carry the literal `convention` label; amendment 1 additions: `t_est` propagation is tested (an evaluator-shifted leaf moves an estimate-quality ancestor's `t_est` while its sound interval is unchanged), and the graded in-band payoff is tested two-sided with MAGNITUDES (`p` slightly above/below `t_est` yields small-magnitude opposite-sign payoffs; `|p − t_est| ≥ 1/κ` saturates at `±1`; all such entries `exact = false` with root `quality = estimate`) — E2.
