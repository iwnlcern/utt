...
- Declared baseline reset (MR2-A; NOT a byte-identical rewrite): the committed hello `elapsed_ms` values are live wall-clock artifacts (X = 22/23/25 ms across the 19 logs; other records carry 56/354 ms), which no honest deterministic clock reproduces. This task therefore performs a ONE-TIME baseline reset: all 19 event logs are regenerated under the manifest's deterministic scripted clock, changing exactly the elapsed-time fields (schema, semantics, replay sufficiency, and every non-timing byte-meaning unchanged; stream files carry no elapsed fields and are unaffected). All 19 changed logs are enumerated in the implementation report. From that reset onward, `--check` byte-compares the full corpus against regeneration. Orchestrator on CC: this declaration is the contract notice; veto before dispatch if a ruling is preferred.
