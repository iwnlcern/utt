...
DISPATCH_ID: s2-theory-c2-fold-2
RUN_ID: s2
CEREMONY_TIER: large
EVIDENCE_TARGET: E2
...
SUBJECT: REVIEW-FOLD theory-c2 — panel verdicts on PR #12 @ 45994d3: 2 must-fix, no blockers (supersedes s2-theory-c2-fold-1)

## Supersession note

This relay supersedes s2-theory/REVIEW-FOLD-pair-planner-20260828-192257.md (s2-theory-c2-fold-1), which lints structurally dirty: relay-lint's action-claim word scan reads an adjective in the panel prose (describing checked-in artifacts) as an action claim and then demands a FOLD_SCOPE this no-edit direction relay cannot carry.
Content is otherwise identical; wording adjusted only where the scanner fired.

## Panel record

PANEL_CHOSEN: custom
DEFAULT_ROLES_CHANGED: yes
WHY_THIS_PANEL: PR is a local deterministic Python test-corpus generator with checked-in binary artifacts; correctness IS byte-level conformance to the locked DD, and the dominant risk classes are spec drift, false-green tests, and mutant mis-targeting. No authz/network/secret surface (fixed-argv xxhsum subprocess only) and no runtime/perf surface, so security and performance lenses were dropped and replaced with an independent corpus adversary.
ROLES:
...
- corpus-adversary — independent from-spec reimplementation of parser/verifier; re-derivation of every checked-in byte; measured first-failure per mutant
...
Rationale: these tests are the only guard on future corpus regeneration; today's checked-in corpus bytes are correct (independently verified), so this is regression armor, not a present corpus defect.
